...
const STATUS_TONE = {
  active: 'bg-sage/15 text-sage-foreground border-sage/30',
  pending_setup:
    'bg-amber-warm/15 text-foreground border-amber-warm/40',
...
const DoctorsTable = () => {
  ...
  const [confirmRemove, setConfirmRemove] = useState(null) // doctor obj
              ...
              {doctors.map((d) => (
                <TableRow key={d._id} className="hover:bg-muted/40">
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={`rounded-full border ${STATUS_TONE[d.accountStatus] || ''}`}
                    >
